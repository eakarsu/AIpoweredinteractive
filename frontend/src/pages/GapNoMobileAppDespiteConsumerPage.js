...
// Gap (Non-AI): No mobile app despite consumer
// No mobile app despite consumer-facing domain
import React, { useState } from 'react';

export default function GapNoMobileAppDespiteConsumerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-mobile-app-despite-consumer/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No mobile app despite consumer</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No mobile app despite consumer-facing domain</p>
