...
// Gap (AI): Services without '/service
// Services without /service-quality-predict (estimate labor hours for repair)
import React, { useState } from 'react';

export default function GapServicesWithoutServicePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-services-without-service/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Services without '/service</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Services without /service-quality-predict (estimate labor hours for repair)</p>
