...
// Gap (AI): Maintenance without '/maintenance
// Maintenance without /maintenance-predict (predict when service will be needed)
import React, { useState } from 'react';

export default function GapMaintenanceWithoutMaintenancePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-maintenance-without-maintenance/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Maintenance without '/maintenance</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Maintenance without /maintenance-predict (predict when service will be needed)</p>
