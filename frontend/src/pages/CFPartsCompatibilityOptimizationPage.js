...
// Custom Feature: Parts compatibility optimization
// Parts compatibility optimization: User inputs repair type; AI recommends parts (OEM vs. aftermarket, compatible alternatives); show price/quality tradeoffs
import React, { useState } from 'react';

export default function CFPartsCompatibilityOptimizationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-parts-compatibility-optimization/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Parts compatibility optimization</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Parts compatibility optimization: User inputs repair type; AI recommends parts (OEM vs. aftermarket, compatible alternatives); show price/quality tradeoffs</p>
