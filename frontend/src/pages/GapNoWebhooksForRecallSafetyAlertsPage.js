...
// Gap (Non-AI): No webhooks for recall/safety alerts
// No webhooks for recall/safety alerts
import React, { useState } from 'react';

export default function GapNoWebhooksForRecallSafetyAlertsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-recall-safety-alerts/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for recall/safety alerts</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for recall/safety alerts</p>
