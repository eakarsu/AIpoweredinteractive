...
// Gap (AI): Recalls without '/recall
// Recalls without /recall-impact-assess (flag recalls affecting user's vehicle)
import React, { useState } from 'react';

export default function GapRecallsWithoutRecallPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-recalls-without-recall/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Recalls without '/recall</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Recalls without /recall-impact-assess (flag recalls affecting user's vehicle)</p>
