...
// User inputs repair type; AI recommends parts (OEM vs. aftermarket, compatible alternatives); show price/quality tradeoffs
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / cf-parts-compatibility-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_parts_compatibility_optimization`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-parts-compatibility-optimization feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Parts compatibility optimization'.
Context: User inputs repair type; AI recommends parts (OEM vs. aftermarket, compatible alternatives); show price/quality tradeoffs
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-parts-compatibility-optimization/run error:', err);
