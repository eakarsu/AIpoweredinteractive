...
// Connect to vehicle OBD2/API; continuous monitoring of diagnostics codes; AI predicts maintenance needs before failure; alerts owner with timing and cost estimates
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / cf-agentic-vehicle-health-monitoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_vehicle_health_monitoring`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-vehicle-health-monitoring feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic vehicle health monitoring'.
Context: Connect to vehicle OBD2/API; continuous monitoring of diagnostics codes; AI predicts maintenance needs before failure; alerts owner with timing and cost estimates
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-vehicle-health-monitoring/run error:', err);
