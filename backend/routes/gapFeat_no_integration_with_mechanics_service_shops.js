...
// Gap Feature: No integration with mechanics/service shops
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / gap-no-integration-with-mechanics-service-shops',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integration with mechanics/service shops'.\nProject: AIpoweredinteractive.\nContext: No integration with mechanics/service shops\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integration-with-mechanics-service-shops', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integration-with-mechanics-service-shops', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
