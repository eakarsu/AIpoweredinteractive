...
// Monitor vehicle VIN against NHTSA/manufacturer recalls; auto-alert if new recall issued; provide recall repair guidance
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / cf-recall-proactive-management',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_recall_proactive_management`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-recall-proactive-management feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Recall proactive management'.
Context: Monitor vehicle VIN against NHTSA/manufacturer recalls; auto-alert if new recall issued; provide recall repair guidance
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-recall-proactive-management/run error:', err);
