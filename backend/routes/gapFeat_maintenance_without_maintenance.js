...
// Gap Feature: Maintenance without \'/maintenance
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / gap-maintenance-without-maintenance',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Maintenance without \'/maintenance'.\nProject: AIpoweredinteractive.\nContext: Maintenance without /maintenance-predict (predict when service will be needed)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['maintenance-without-maintenance', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'maintenance-without-maintenance', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
