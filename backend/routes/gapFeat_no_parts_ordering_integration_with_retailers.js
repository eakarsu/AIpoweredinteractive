...
// Gap Feature: No parts ordering (integration with retailers)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / gap-no-parts-ordering-integration-with-retailers',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No parts ordering (integration with retailers)'.\nProject: AIpoweredinteractive.\nContext: No parts ordering (integration with retailers)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-parts-ordering-integration-with-retailers', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-parts-ordering-integration-with-retailers', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
