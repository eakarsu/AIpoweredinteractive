...
// Gap Feature: No real vehicle API integration (BMW ConnectedDrive, Tesla API, OBD2)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / gap-no-real-vehicle-api-integration-bmw-connecteddrive',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No real vehicle API integration (BMW ConnectedDrive, Tesla API, OBD2)'.\nProject: AIpoweredinteractive.\nContext: No real vehicle API integration (BMW ConnectedDrive, Tesla API, OBD2)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-real-vehicle-api-integration-bmw-connecteddrive', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-real-vehicle-api-integration-bmw-connecteddrive', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-real-vehicle-api-integration-bmw-connecteddrive']
    );
