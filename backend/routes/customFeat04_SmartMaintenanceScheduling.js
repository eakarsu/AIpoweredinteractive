...
// Predict maintenance (oil change, tire rotation, filter replacement) based on mileage and time; integrate with dealer/shop scheduling; book appointments
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / cf-smart-maintenance-scheduling',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_smart_maintenance_scheduling`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-smart-maintenance-scheduling feature
router.post('/run', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Smart maintenance scheduling'.
Context: Predict maintenance (oil change, tire rotation, filter replacement) based on mileage and time; integrate with dealer/shop scheduling; book appointments
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-smart-maintenance-scheduling/run error:', err);
