...
// Gap Feature: Recalls without \'/recall
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIpoweredinteractive / gap-recalls-without-recall',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Recalls without \'/recall'.\nProject: AIpoweredinteractive.\nContext: Recalls without /recall-impact-assess (flag recalls affecting user\'s vehicle)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['recalls-without-recall', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'recalls-without-recall', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
